...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `addDocument`(
    ...
    `const_residencia` VARCHAR(50), `manifiesto` VARCHAR(50), `capacidad_economica` VARCHAR(50), `cedula_rfc` VARCHAR(50), `tipo_registro` INT, OUT id int)
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `addComunidad`(
	IN `municipio` VARCHAR(150), 
    ...
    IN num_emision VARCHAR(45),
	IN ocr VARCHAR(256),
	IN cic VARCHAR(256),
	IN seccion VARCHAR(256),
...
BEGIN
	...
	INSERT INTO candidatura_comunidad VALUES( null, nombre, apellido_paterno, apellido_materno, genero, partido_politico, municipio, id_comunidad, candidatura, clave_elector, curp, num_emision, ocr, cic, seccion, fec_nacimiento, lugarnacimiento,
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `addDiputacion`(
	IN `nombre` VARCHAR(150), 
    IN `apellido_paterno` VARCHAR(150), 
    IN `apellido_materno` VARCHAR(150), 
    IN `genero` VARCHAR(50), 
    ...
    IN reg_pro_num_emision varchar(254),
	IN reg_pro_ocr varchar(254),
	IN reg_pro_CIC varchar(254),
	IN reg_pro_seccion varchar(254),
	...
	IF partido IS NULL THEN
        ...
        IF clave IS NULL THEN
            ...
            IF curp2 IS NULL THEN
                    ...
                    fec_nacimiento, lugar_nacimiento, clave_elector, curp, num_emision, ocr, cic, seccion, tiempo_residencia, estado, user_capturista, sobrenombre, 1, id_tipo_registro, politicaPrivacidad );
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addMunicipal`(
    ...
    IN num_emision VARCHAR(150),
	IN ocr VARCHAR(150),
	IN cic VARCHAR(150),
	IN seccion VARCHAR(150),
...
BEGIN
    ...
    ELSEIF tipoEleccion = 4 THEN
		INSERT INTO candidatura_ayuntamiento VALUES( null, nombre, apellido_paterno, apellido_materno, genero, partido_politico, id_municipio, tipoEleccion, clave_elector, curp, num_emision, ocr, cic, seccion, fec_nacimiento, lugarnacimiento,
				tiempo_residencia, sobrenombre, estado, user_capturista, propietario_suplente, id_documento, politicaP);
		call addHistorial(user_capturista, tipo, nombre_clv, clave_elector, ' ' , ' ', ' ' , dia, hora, tipoEleccion);
		select 1 as MENSAJE;
	END IF;
...
UNION ALL
    ...
    '' as prelacion
    FROM candidatura_ayuntamiento, partidos_municipio, estados, tipo_eleccion, municipios
    ...
    AND partidos_municipio.id_partido_municipio = candidatura_ayuntamiento.partido_politico 
...
UNION ALL
    ...
    '' as prelacion
    FROM candidatura_sindicatura, partidos_municipio, estados, tipo_eleccion, municipios
    ...
    AND partidos_municipio.id_partido_municipio = candidatura_sindicatura.partido_politico 
...
UNION ALL 
    ...
    '' as prelacion
    FROM candidatura_regiduria, partidos_municipio, estados, tipo_eleccion, municipios
    WHERE candidatura_regiduria.clave_elector = clave_elector 
	AND municipios.id_municipio = candidatura_regiduria.id_municipio
    AND partidos_municipio.id_partido_municipio = candidatura_regiduria.partido_politico 
...
SELECT 
    ...
    '' as prelacion
    FROM candidatura_comunidad, partidos_comunidad, estados, tipo_eleccion, municipios
    WHERE candidatura_comunidad.clave_elector = clave_elector
    AND municipios.id_municipio = candidatura_comunidad.id_municipio
    AND  partidos_comunidad.id_partido_comunidad = candidatura_comunidad.partido_politico 
...
BEGIN
    ...
    SELECT 
		...
		FROM candidatura_regiduria, tipo_eleccion, partidos_municipio, municipios
		WHERE candidatura_regiduria.partido_politico = partidos_municipio.id_partido_municipio
        AND municipios.id_municipio = candidatura_regiduria.id_municipio
		AND tipo_eleccion.id_eleccion = candidatura_regiduria.candidatura
...
UNION ALL
	SELECT
		...
		FROM candidatura_regiduria, partidos_municipio, estados, tipo_eleccion
		WHERE candidatura_regiduria.clave_elector = clave_elector 
		AND candidatura_regiduria.partido_politico = partidos_municipio.id_partido_municipio
...
BEGIN
	...
	UNION ALL
		...
		candidatura_regiduria.estado,
		candidatura_regiduria.id_documento
		FROM candidatura_regiduria, tipo_eleccion, estados, partidos_municipio 
		WHERE candidatura_regiduria.clave_elector = clave_elector AND partidos_municipio.id_partido_municipio = candidatura_regiduria.partido_politico AND 
...
BEGIN
	SELECT nombre, apellido_paterno, apellido_materno, genero, propietario_suplente, nombre_partido_politico, clave_elector, nombre_distrito, nombre_candidatura, estado
		FROM ( 
			SELECT candidatura_gubernatura.nombre, 
			candidatura_gubernatura.apellido_paterno, 
			candidatura_gubernatura.apellido_materno,
			candidatura_gubernatura.genero, 
			'Propietario' as propietario_suplente, 
	...
	UNION ALL
			...
			candidatura_renuncia.estado
			FROM candidatura_renuncia, partidos_gubernatura, tipo_eleccion
			WHERE partidos_gubernatura.id_partido_gubernatura = candidatura_renuncia.partido_politico
...
BEGIN
	...
	ELSEIF id_tipo_candidatura = 4 OR id_tipo_candidatura = 6 OR id_tipo_candidatura = 7 THEN 
