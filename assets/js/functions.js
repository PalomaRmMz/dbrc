...
function validacionSexo(data, contenedor) {
    ...
    sltPost('servicios/diputaciones/validarSexo.php', data).then((respuesta) => {
        ...
        if (respuesta.status_code == 201) {
            ...
            swal("Información", 'Ya se han registrado el propietario y suplente', 'info');
            return respuesta;
        }
        $(`#${contenedor}`).removeClass("d-none");
...
let validarCampos = async(formulario) => {
        ...
        for (let i = 0; i < form1Inputs.length - 1; i++) {
            if ($(`#${form1Inputs[i].id}`).val() == "") {
                ...
                if (campo == "Apellido Paterno" || campo == "Correo Electrónico" || campo == "CIC (Código de Identificación de Credencial para Votar)" || campo == "Sobrenombre" ) continue;
...
function mensaje() {
    swal('Se ha iniciado sesión en otro dispositivo', 'Se cerrara la sesión actual', 'info');
}
...
function generarPDF(claveOriginal, documento, candidatura) {
    var win = window.open(pathPdfs + documento + '.php?name=' + claveOriginal + '&candidatura=' + candidatura, '_blank');
    if (win) {
        win.focus();
    } else {
        alert('Porfavor habilíta las ventanas emergentes');
...
$(document).ready(function() {
  ...
  /* $("#reg-distritos").change(async function(data) {
    ...
    $(".add-user").click(function() {
        ...
        $('#btns-save').html('<button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button> <button type="button" class="btn btn-primary" id="btn_nuevoUsuario">Añadir Usuario</button>');
