$(document).ready(function () {
    ...
    $.ajax({
      ...
      .done(function (respuesta) {
        if (respuesta[0].datos == 0) {
          swal(
            "Error",
            "No se pudo conectar a la base de datos de usuario",
            "error"
          );
        } else if (respuesta[0].datos == 1) {
          swal(
            "Error",
            "No hay registro en la base de datos de usuario",
            "error"
          );
        } else {
          var fila = "";
          ...
          for (var i = 0; i < respuesta.length; i++) {
            fila +=
              ...
              respuesta[i].rol +
              "</td>" +
              "<td> <button type='button' class='btn btn-primary btn-sm edit-user' data-user='" +
              respuesta[i].usuario +
              "'> <i class='fa fa-pencil-square-o' aria-hidden='true'></i></button> <button type='button' class='btn btn-secondary btn-sm eliminar-usuario' data-user='" +
              respuesta[i].usuario +
  ...
  $.nuevoUsuario = function (
    ...
    $.ajax({
      ...
      data: {
        nombre: username,
        apellidoPaterno: userapp,
        apellidoMaterno: userapm,
        nombreDUsuario: userusername,
      ...
      .done(function (respuesta) {
        ...
        if (respuesta.datos == 0) {
          ...
          swal(
            "Usuario agregado",
            "El registro se almaceno exitosamente",
            "success"
          );
          $.listarUsuarios();
        }
    ...
    $.ajax({
      ...
      .done(function (respuesta) {
        ...
        if (respuesta.datos == 0) {
          ...
          $("#btns-save").html(
            '<button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button> <button type="button" class="btn btn-primary" id="btn_editUsuario">Editar Usuario</button>'
          );
  ...
  $(document).on("click", "#btn_nuevoUsuario", function () {
    ...
    if (
      ...
      swal(
        "Error de Registro",
        "Llena toda la información para ingresar un nuevo usuario",
        "error"
      );
    } else {
