...
function llenarSubsanacion(respuesta) {
    ...
    } else if (respuesta[0].candidatura == 'GUBERNATURA' || respuesta[0].candidatura == "COMUNIDAD") {
    ...
    } else if (respuesta[0].id_candidatura == TE_SINDICATURA || respuesta[0].id_candidatura == TE_REGIDURIA) {
...
$.printInfoCandidatoSubDoble = function(clave_elector, id_registro) {
    respuesta = registrosSubsanacion.filter(item => item.id_candidatura == id_registro && item.clave_elector == clave_elector);
    llenarSubsanacion(respuesta);
}
...
$.printSubDocs = function(clave_elector, id_documento) {
    $.ajax({
        ...
        if (respuesta[0].datos == 0) {
            ...
            $("input[name=actaSubsanacion][value='" + respuesta[0].COPIA + "']").prop('checked', true);
            $("input[name=subsanacionSep][value='" + respuesta[0].SEPARACION + "']").prop('checked', true);
...
$(".btn-update-modal").click(function() {
    ...
    infoSubsanacion.candidato_fec_nacimiento_rp = $("#candidato_fec_nacimiento-rp").val();
    infoSubsanacion.candidato_genero_rp = $("#candidato_genero-rp").val();
    infoSubsanacion.candidato_clave_elector_rp = $("#candidato_clave_elector-rp").val();
    infoSubsanacion.candidato_curp_rp = $("#candidato_curp-rp").val();

    infoSubsanacion.candidato_num_emision_rp = $("#candidato_num_emision-rp").val();
    ...
    infoSubsanacion.candidato_tiempo_residencia_rp = $("#candidato_tiempo_residencia-rp").val();
    infoSubsanacion.candidato_sobrenombre_rp = $("#candidato_sobrenombre-rp").val();
   ...
   infoSubsanacion.consVinculoComuni = 'NO APLICA';
    ...
    variable = documentacionValidacion(
       ...
       infoSubsanacion.cartaEleccion,
        ...
        infoSubsanacion.subSNR, infoSubsanacion.subREG, infoSubsanacion.subCRED, infoSubsanacion.subACTA, infoSubsanacion.subCOPIA, infoSubsanacion.subCONSTANCIA, infoSubsanacion.subACEPTACION,
        infoSubsanacion.subSEPARACION, infoSubsanacion.subESCRITO, infoSubsanacion.subCARTA, infoSubsanacion.subMANIFESTACION, infoSubsanacion.subCAPACIDAD, infoSubsanacion.subCEDULA);
    ...
    } else if (idTipoCandidatura == TE_MUNICIPAL || idTipoCandidatura == TE_SINDICATURA || idTipoCandidatura == TE_REGIDURIA) {
...
function postSubsanacion(url, dataSubsanacion, documents) {
    ...
    ({ subEstado, candidato_clave_elector_rp, usuarioActual, idTipoCandidatura, subSEPARACION, candidato_pp } = dataSubsanacion);
    ...
    $.ajax({
                ...
                if (subEstado !== "Completo") {
                    ...
                    if (candidato_pp == "Candidatura Independiente") {
                        ...
                        if (subSEPARACION == "NO") {
                            ...
                            generarPDF(candidato_clave_elector_rp, docFaltantesCIndp, idTipoCandidatura);
                            generarPDF(candidato_clave_elector_rp, docRenuncia, idTipoCandidatura);
                        ...
                        if (subSEPARACION == "NO") {
                            generarPDF(candidato_clave_elector_rp, docFaltantesPP, idTipoCandidatura);
                            generarPDF(candidato_clave_elector_rp, docRenuncia, idTipoCandidatura);
                        } else {
                            generarPDF(candidato_clave_elector_rp, docFaltantesPP, idTipoCandidatura);
                        }
                    ...
                    generarCV(candidato_clave_elector_rp, usuarioActual, allDocument, idTipoCandidatura);
                }

                swal("Modificación exitosa", "El registro se modificó exitosamente", "success")
        ...
        swal("Modificación exitosa", "No se realizo ningun cambio", "success")
        resetInputs()
...
$.actualizarInfoComunidad = function(infoSubsanacion) {
    ...
    $.ajax({
        ...
        try {
            var respuesta = JSON.parse(respuesta)
            if (respuesta.datos == 0) {
                swal("ERROR", "No se pudo conectar a la base de datos de candidatos", "warning")
                resetInputs()
            } else if (respuesta.datos == 1) {
                ...
                if (cambios.length > 0) {
                    ...
                    if (subEstado == "Completo") {
                        ...
                        generarCV(candidato_clave_elector_rp, usuarioActual, documento, idTipoCandidatura);
