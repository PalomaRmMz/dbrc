<?php
  ...
  if ($_POST) {
    ...
    $stmtclv = $DB_con->prepare("SELECT * FROM candidatura_regiduria WHERE clave_elector = '".$clv."' AND candidatura =  $candidatura ");
    $stmtclv->execute();
    $res1 = $stmtclv->get_result();
    $stmtcurp = $DB_con->prepare("SELECT * FROM candidatura_regiduria WHERE curp = '".$curp."' ");
    $stmtcurp->execute();
    ...
    if($res1->num_rows > 0){
      ...
      $stmtstatus = $DB_con->prepare("UPDATE candidatura_renuncia SET estado = 'Sustitucion' WHERE clave_elector = '".utf8_decode($claveOriginal)."' AND candidatura = 7 ");
      $stmtstatus->execute();


      $stmt = $DB_con->prepare("UPDATE candidatura_regiduria SET 
         nombre = '".$nombre."',
      ...
      $historial = array($usuario, "Sustitucion", $nombreOriginal.' '.$appOriginal.' '.$apmOriginal, $claveOriginal, $nombre.' '.$app.' '.$apm, $clv, $motivoSustitucion, $dia, 
      $hora, $candidatura);
