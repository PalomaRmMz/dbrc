<?php
  ...
  if ($_POST) {
    ...
    $stmtclv = $DB_con->prepare("SELECT * FROM candidatura_comunidad WHERE clave_elector = '".$clv."' AND candidatura =  $candidatura ");
    $stmtclv->execute();
    $res1 = $stmtclv->get_result();

    $stmtcurp = $DB_con->prepare("SELECT * FROM candidatura_comunidad WHERE curp = '".$curp."' ");
    $stmtcurp->execute();
    ...
    if($res1->num_rows > 0){
      ...
      INSERT INTO candidatura_renuncia( nombre, apellido_paterno, apellido_materno, genero, partido_politico, candidatura,  clave_elector, curp, num_emision, ocr, cic, seccion, distrito, municipio, comunidad,
      fec_nacimiento, tiempo_residencia, sobrenombre, estado, user_capturista, propietario_suplente, prelacion, fecha_registro )
        
      SELECT nombre, apellido_paterno, apellido_materno, genero, partido_politico, candidatura, clave_elector, curp, num_emision, ocr, cic, seccion,
     'Circunscripción única'  as distrito, id_municipio, id_comunidad, fec_nacimiento, tiempo_residencia,
      ...
      $stmtstatus = $DB_con->prepare("UPDATE candidatura_renuncia SET estado = 'Sustitucion' WHERE clave_elector = '".utf8_decode($claveOriginal)."' AND candidatura = 5 ");
      $stmtstatus->execute();



      $stmt = $DB_con->prepare("UPDATE candidatura_comunidad SET 
         nombre = '".$nombre."',
      ...
      $queryHistory = 
      "INSERT INTO historial VALUES(null, '$usuario', 'Sustitucion', $nombreCompleto , '$claveOriginal', '".$nombre.' '.$app.' '.$apm."', '$clv', '$curp' ,'$motivoSustitucion', '$dia', 
      '$hora', '$candidatura', '$id_partido', '$id_distrito', '$id_municipio', '$id_comunidad', '$prop_sup', '$estado' );";
      $stmthistory= $DB_con->prepare($queryHistory);
      $stmthistory->execute();
