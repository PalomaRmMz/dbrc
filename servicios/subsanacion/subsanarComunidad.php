   ...
   // fin nuevos campos
    ...
    $stmt= $DB_con->prepare("
      ...
      SET
        ...
        $stmt2 = $DB_con->prepare($queryUpdate);
        ...
        $stmtdoc->execute();
        // Actualiza la clave de elector cuando se subsana un la clave de elector
         $stmthistorial= $DB_con->prepare("UPDATE historial SET clv_original = '".utf8_decode($candidato_clave_elector)."' WHERE clv_original = '".$claveOriginal."'  AND id_tipo_eleccion = ".$idTipoCandidatura." ");
         $stmthistorial->execute();
         // Guarda en Historial 


         $nombreCompleto = $candidato_nombre.' '.$candidato_ap_paterno.' '.$candidato_ap_materno;
         $query = "INSERT INTO historial values(null, '$usuario', '$tipoAccion', '$nombreCompleto' , '$candidato_clave_elector',  '-',  '-', '$candidato_curp', '-', '$dia', 
         '$hora', '$idTipoCandidatura', '$id_partido','$id_distrito', '$id_municipio', '$id_comunidad', '$propietario_suplente', '$estado' )";
