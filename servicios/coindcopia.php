...
date_default_timezone_set('America/Mexico_city');
  ...
  $stmt = $DB_con->prepare("SELECT
      ...
      sobrenombre,
      user_capturista
    FROM candidatura_diputado WHERE clave_elector = '".$clave_elector."' ");
  $stmt->execute();
  $res = $stmt->get_result();
  ...
  if($res->num_rows < 1 ) {
    ...
    // $has = md5(date("h:i:sa"));
    // $st = $DB_con->prepare('INSERT INTO  registro(tipo_doc,usuario,fecha,has)VALUES("coind","'.$candidatos["user_capturista"].'","'.date('Y-m-d').'","'.$has.'");');
    //   $st->execute();

    $str = $DB_con->prepare("SELECT * FROM registro WHERE clave_original =  '".$clave_elector."' AND tipo_doc ='coind' ORDER BY id DESC LIMIT 1 ");
    $str->execute();
    $res = $str->get_result();
