...
FROM candidatura_diputado WHERE clave_elector = '".$clave_electores."' ");
   ...
   $st = $DB_con->prepare('INSERT INTO  registro(tipo_doc,usuario,nombre,app,apm,fecha,hora,has,clave_original)VALUES("RSCARGO","'.$candidatos["user_capturista"].'"," "," "," ","'.date('Y-m-d').'","'.date('H:i:s').'","'.$has.'","'.$clave_elector.'");');
     $st->execute();


     $str = $DB_con->prepare("SELECT * FROM registro WHERE has =  '".$has."' ");
     $str->execute();
