...
date_default_timezone_set('America/Mexico_city');
 ...
 $clave_elector = $_GET["name"];
  ...
  $stmt = $DB_con->prepare("SELECT
      ...
      user_capturista,
      prelacion
    FROM candidatura_diputado WHERE clave_elector = '".$clave_elector."' AND candidatura = 'REPRESENTACIÓN PROPORCIONAL' ");
  $stmt->execute();
 ...
 $stmtUsu = $DB_con->prepare("SELECT
      nombre_capturista, app_capturista, apm_capturista FROM usuarios WHERE usuario = '".$capturista."' ");
    $stmtUsu->execute();
   ...
   //    $has = md5(date("h:i:sa"));
   // $st = $DB_con->prepare('INSERT INTO  registro(tipo_doc,usuario,fecha,has,clave_original)VALUES("cvind","'.$candidatos["user_capturista"].'","'.date('Y-m-d').'","'.$has.'","'.$clave_elector.'");');
   //   $st->execute();
...
$pdf = new PDF();
  ...
  $pdf->Text(11,42,utf8_decode('FOLIO ' .utf8_decode($aaa['id']).''));
	...
	$pdf->Ln(1);
	$pdf->Cell(70,12,utf8_decode(' -           Copia Cértificada del Acta de nacimiento (extracto);') );
	$pdf->Ln(1);
	...
	$pdf->Ln(2);
	$pdf->SetFont('Arial','',7);
	$pdf->MultiCell(180,3, utf8_decode('Decimo: de la revisión realizada a todos los docuemtos citados, se advierte que el (la) cuidadano (a) da cumplimiento a cada uno de los requisistos enlistados en el artículo 35 de la Constitución Política del estado Libre y Soberano de Tlaxcala.'),0,'J', 0 );
	$pdf->Ln(2);
	$pdf->SetFont('Arial','',7);
