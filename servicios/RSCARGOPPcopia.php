...
FROM candidatura_diputado WHERE clave_elector = '".$clave_electores."' ");
   ...
   // $st = $DB_con->prepare('INSERT INTO  registro(tipo_doc,usuario,fecha,has)VALUES("rscargopp","'.$candidatos["user_capturista"].'","'.date('Y-m-d').'","'.$has.'");');
   //   $st->execute();

     $str = $DB_con->prepare("SELECT * FROM registro WHERE clave_original =  '".$clave_elector."'AND tipo_doc ='RSCARGOPP' ORDER BY id DESC LIMIT 1 ");
     $str->execute();
