<?php
  ...
  if($tipo_candidatura == 6){
    ...
    $query = "SELECT
		...
		WHERE clave_elector = '$clave_elector' AND candidatura = $tipo_candidatura
    ...
    sobrenombre,
    user_capturista
    FROM candidatura_ayuntamiento WHERE clave_elector = '".$clave_elector."' AND candidatura = $tipo_candidatura ";
  }
  ...
  if($res->num_rows < 1 ) {
    ...
    $stmtdoc = $DB_con->prepare("SELECT * FROM candidatura_documentado WHERE clave_elector = '".$clave_electores."' AND  id_tipo_registro = $tipo_candidatura ");
    $stmtdoc->execute();
...
//	header("Content-Type: text/html; charset=iso-8859-1 ");
    ...
    $pdf->Ln(40);
	$pdf->SetFont('Arial','B',10);
    $pdf->Cell(100,0,utf8_decode('Candidatura Independiente'));
	$pdf->Ln(6);
	$pdf->SetFont('Arial','B',10);
