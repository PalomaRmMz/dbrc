    ...
    if($result){
      ...
      if($result){
       $respuesta = array( 'mensaje' => 'Ya existe un registro con esa clave elector o curp en candidatura para ayuntamiento', 'status' => false );
        ...
        $stmtclv = $DB_con->prepare("SELECT * FROM candidatura_diputado WHERE clave_elector = '".utf8_decode($clv)."' AND candidatura = '2' ");
        $stmtclv->execute();
        $res1 = $stmtclv->get_result();
    
        $stmtcurp = $DB_con->prepare("SELECT * FROM candidatura_diputado WHERE curp = '".$curp."' AND candidatura = '2' ");
        $stmtcurp->execute();
        $res2 = $stmtcurp->get_result();
    
        $stmtpp = $DB_con->prepare("SELECT * FROM candidatura_diputado WHERE partido_politico = '".utf8_decode($pp)."' AND propietario_suplente = '".utf8_decode($prosup)."' AND prelacion = '".utf8_decode($prelacion)."' ");
        $stmtpp -> execute();
        $res3 = $stmtpp->get_result();
        ...
        if ($res1->num_rows == 1) {
          ...
          $respuesta = array( 'mensaje' => 'Ya existe un propietario de este partido politico con este numero de prelacion', 'status' => false );
        } else {
